...
public class ApplicationController {
    ...
    public ResponseEntity<?> candidate(
        ...
        ResponseEntity<?> denied = authorizeCandidate(email, authorization);
        ...
        try {
            return ResponseEntity.ok(service.findByCandidate(email));
    ...
    public ResponseEntity<?> apply(
        ...
        String email = application == null ? null : application.getCandidate();
        ...
        try {
            return ResponseEntity
                    ...
                    .body(service.create(application));
            ...
            if (e.getMessage().equals("Job not found")) {
                return ResponseEntity
    ...
    /*
     * Recruiter: view all applications for a particular job
     */
    @GetMapping("/job/{jobId}")
    public ResponseEntity<?> jobApplications(@PathVariable Long jobId) {
        return ResponseEntity.ok(applications.findByJobId(jobId));
    }
    ...
    public ResponseEntity<?> status(
        ...
        try {
            ...
            return ResponseEntity
                    .status(
                            e.getMessage().equals("Application not found")
...
public ResponseEntity<?> recruiterStatistics(
    ...
    private ResponseEntity<?> authorizeCandidate(
            String email,
        ...
        JwtService.Claims claims =
                jwt.read(authorization.substring(7).trim());
        ...
        if (!"CANDIDATE".equalsIgnoreCase(claims.role())) {
            return ResponseEntity
                    .status(HttpStatus.FORBIDDEN)
                    .body("Candidate access is required");
        }

        if (email == null ||
                !claims.email().equalsIgnoreCase(email)) {

            return ResponseEntity
                    .status(HttpStatus.FORBIDDEN)
                    .body("You can only access your own applications");
        }
