...
public class AuthController {
    ...
    private final com.klu.auth_service.security.JwtService jwt;
    public AuthController(AuthService service, com.klu.auth_service.security.JwtService jwt) { this.service = service; this.jwt = jwt; }
    ...
    @PutMapping("/recruiter-request") public ResponseEntity<?> recruiterRequest(@RequestHeader(value="Authorization",required=false) String header) { try { if(header==null||!header.startsWith("Bearer ")) return ResponseEntity.status(401).body("Login required"); return ResponseEntity.ok(service.requestRecruiter(jwt.email(header.substring(7)))); } catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); } }
